...
def partyByDisplayName(parties):
    return {p['display_name']: p['party'] for p in parties}

def list_parties():
    result = subprocess.run([
        "daml", "ledger", "list-parties", "--json"
    ], check=True, capture_output=True, text=True)
    return partyByDisplayName(json.loads(result.stdout))    
